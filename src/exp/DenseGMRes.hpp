 ...
 *
 */
// #ifndef DENSEGMRES_HPP
#define DENSEGMRES_HPP
#include <iostream>
#include <iomanip>
...
namespace strumpack {
  ...
  DenseGMRes
  (
   std::function<void(const scalar_t*,scalar_t*)>& spmv,
   ...
   )
  {
    ...
    int ldh = restart+1;
    scalar_t rho;
    scalar_t rho0 = scalar_t(0.);
    blas::copy(n, b, 1, b_prec, 1);
    preconditioner(b_prec);

    bool no_conv = true;
    int totit = 0;
    std::vector<scalar_t> convergence;
      ...
      for (int it=0; it<restart; it++) {
        ...
        if (verbose)
          std::cout << "GMRES it. " << totit << "\tres = "
                    ...
                    << "\trel.res = " << std::setw(12)
                    << rho/rho0 << std::endl;
        convergence.push_back(rho/rho0);
