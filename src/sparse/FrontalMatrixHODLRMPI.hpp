...
namespace strumpack {
  ...
  FrontalMatrixHODLRMPI<scalar_t,integer_t>::release_work_memory() {
    std::cout << "TODO, release work memory" << std::endl;
  }
  ...
  (const SpMat_t& A, const Opts& opts, int etree_level, int task_depth) {
    ...
    std::cout << "TODO construct HODLR matrix for this front" << std::endl;
    ...
    if (dim_sep()) {
      if (etree_level > 0) {
        ...
        std::cout << "TODO partial HODLR factorization" << std::endl;
        ...
        //_ULV = _H->factor();
        std::cout << "TODO HODLR factorization of root front" << std::endl;
        TIMER_STOP(t_fact);
      }
    }
  }
