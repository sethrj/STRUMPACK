...
int run(int argc, char *argv[]) {
  ...
  // locfile="frouet/Airbus/sphereAcous10002.res";
  // prefix="/project/projectdirs/m2957/liuyangz/my_research/matrix/";  
  locfile="sphereAcous10002.res";
  prefix="/home/administrator/Desktop/research/matrix/";
  ...
  // case 'U': { // upper triangular Toeplitz
    ...
    // for (int j=0; j<m; j++)
      // for (int i=0; i<m; i++)
        // if (i > j) Aseq(i, j) = 0.;
        // else Aseq(i, j) = (i==j) ? 1. : 1./(1+abs(i-j));
  // } break;
  // case 'L': {
    // if (argc > 2) m = stoi(argv[2]);
    // if (argc <= 2 || m < 0) {
	...
	// Aseq = DenseMatrix<myscalar>(m, m);
    // Aseq.eye();
    // DenseMatrix<myscalar> U(m, max(1, int(0.3*m)));
    // DenseMatrix<myscalar> V(m, max(1, int(0.3*m)));
    // U.random();
    // V.random();
    // gemm(Trans::N, Trans::C, 1./m, U, V, 1., Aseq);
  // } break;
  ...
  if(!myid) {
    ...
    fp.read((char*)Aseq.data(), 16*Aseq.rows()*Aseq.cols());
    fp.close();
    Aseq.print();
  }
  tend=MPI_Wtime();
