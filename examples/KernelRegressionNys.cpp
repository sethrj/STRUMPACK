...
int main(int argc, char *argv[]) {
  ...
  cout << "# usage: ./KernelRegressionNys file d h lambda "
  ...
  cout << "# M (Nystrom)      = " << M << std::endl;
  cout << "# gmres_it         = " << gmres_it << std::endl;
  cout << "# gmres_rtol       = " << gmres_rtol << std::endl << std::endl;
  ...
  cout << "## Clustering took: " << timer.elapsed() << std::endl << std::endl;
  ...
  cout << std::endl << "# Factorization..." << std::endl;
  timer.start();
  auto ULV = H.factor();
  cout << "## Factorization took: " << timer.elapsed() << std::endl;

  cout << std::endl << "# Solve..." << std::endl;
  timer.start();
  // Operation: z = (KnM') * y;
  ...
  // Using preconditioner
    ...
    auto iter_wrapper = [&](const std::function<void(scalar_t*)>& prec) {
      scalar_t opts_rel_tol       = gmres_rtol;
      scalar_t opts_abs_tol       = 1e-20;
      int      opts_maxit         = gmres_it;
      ...
      cout << "## Krylov iterations: " << totit << std::endl;
    };
  ...
  std::cout << "## Forming matrix for predictions..." << std::endl;
  timer.start();
  DenseMatrix<scalar_t> Kr(m,M);
  for (int c = 0; c < m; c++)
    for (int r = 0; r < M; r++)
      Kr(c, r) = Gauss_kernel(&testing[c*d], &training[Mid[r]*d], d, h);
  scalar_t time_form_matrix = timer.elapsed();
  cout << "## Forming matrix elapsed:    " << time_form_matrix << std::endl;
  ...
  scalar_t time_matvec = timer.elapsed();
  cout << "## Prediction matvec elapsed: " << time_matvec << std::endl;
  ...
  cout << "## Prediction took:           " << time_form_matrix + time_matvec << std::endl;
